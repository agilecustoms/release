...
1. [conventional commits](./conventional-commits.md) (default) — next version determined based on commit message prefixes
such as `fix:`, `feat:`, `docs:`, `perf:`. This mode allows to use all features of this action (see table below)
2. [version bump](#version-bump) — (when pass `version-bump` input) allows to bump a version w/o special commit messages, useful in enterprise 
3. [explicit version](#explicit-version) — (when pass `verion` input) provide a new version as explicit input

| feature              | conventional commits | version bump | explicit version |
|----------------------|----------------------|--------------|------------------|
...
In enterprise the software is being developed, deployed and tested by the same team, so teams often do not use conventional commits.
All that matters — the version is bumped, artifacts are published. For such teams there is an option `version-bump`,
it allows to bump a minor/patch version even if there are no special prefixes in commit messages

`version-bump` can take the following values:
- (no-value) — default, meaning use "conventional commits" mode
- `default-patch` — if no conventional commits, default to patch bump
- `default-minor` — if no conventional commits, default to minor bump
- (planned) `patch` — bump a patch version, ignore conventional commits
- (planned) `minor` — bump a minor version, ignore conventional commits

⚠️ Notes:
- CHANGELOG.md updated / GH release created only if there is a conventional commit(s)
...
Alternatively to "conventional commits" and "version bump", you can provide an explicit version to be used for the release
...
("conventional commits" and "version bump" take it from `.releaserc.json` file)
