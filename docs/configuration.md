...
`agilecustoms/release` uses npm library "semantic-release" in _dryRun_ mode — just to generate next version and release notes.
...
so they take configuration as per semantic-release documentation to an extent that _dryRun_ mode supports.
...
In addition to a semantic-release configuration file, `agilecustoms/release` takes configuration via [inputs](../README.md#inputs).
...
| `plugins`                                                                                                   | `release-plugins`  | mainly for [conventional commits](features/conventional-commits.md)                                          |
